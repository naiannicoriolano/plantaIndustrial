...
#define LEITURAS_ESPERADAS 2000 
...
// Le arquivo bruto, organiza, ordena e salva os dados.
int main(int argc, char *argv[]) {
    ...
    while (fscanf(infile, "%ld %49s %99[^\n]", &ts_long, current_id, current_val) == 3) {
        ...
        if (sensor_idx == -1) {
            if (num_sensores_unicos >= MAX_SENSORS) {
                printf("Numero maximo de sensores (%d) atingido. Ignorando %s.\n", MAX_SENSORS, current_id);
                continue;
            }
    ...
    for (int i = 0; i < num_sensores_unicos; i++) {
        // Verifica se a contagem de leituras e diferente do esperado.
        if (sensores[i].count != LEITURAS_ESPERADAS) {
            printf("Aviso: O sensor '%s' possui %d leituras (esperado: %d).\n", 
        ...
        fclose(outfile); 
        printf("Dados do sensor '%s' organizados e salvos em '%s'\n", sensores[i].id_sensor, out_filename);
